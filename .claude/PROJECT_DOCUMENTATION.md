...
1b. Fails the trade (`MARKET_CLOSED`) if `snapshot.marketStatus !== 'TRADEABLE'`, even when bid/offer are present (added 2026-07-16: a real TSLA order was rejected by IG for "Market closed," but `confirmDeal` only returned the unhelpful `error.confirms.deal-not-found` — IG's confirms endpoint apparently never creates a trackable deal for a gateway-level rejection like this, so it had nothing real to report back. Checking `marketStatus` upfront, using data already fetched for scaling, catches this with the real reason instead of discovering it via a vague confirm failure). A closed market can still show last-known bid/offer values, so this check is separate from and after the `NO_LIVE_QUOTE` one above.
...
**Retries with a short delay (added the same day, after OSCR still failed post-deploy).** The first version checked `GET /positions` exactly once, immediately. That's not enough — IG's own position propagation can lag behind an ambiguous `confirmDeal` response by up to roughly a second, so a single immediate check can still see "not there yet" for a position that appears moments later (confirmed live: an OSCR BUY filled on IG, but a same-instant reconciliation attempt still missed it). `reconcileAgainstOpenPositions` now retries up to `RECONCILE_MAX_ATTEMPTS` (3) times, `RECONCILE_RETRY_DELAY_MS` (500ms) apart, before giving up. This never risks the webhook's 3-second deadline — `WebhookController.receiveSignal` never awaits `processSignal()`, so the whole pipeline including this retry runs after the webhook has already responded 200. The dev test-signal endpoint does await it, so a worst-case ambiguous confirm adds ~1 second there — acceptable for a manual test tool.

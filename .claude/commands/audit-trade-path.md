...
3. **Quantity calculation** (`trade/`) — `Math.floor(investment_amount / signal_price)`, whole shares only, guarded against divide-by-zero and against flooring to zero (must still log FAILED, not throw unhandled — verify it's inside the same try/catch as IG errors, not before it). `investment_amount` must be the *resolved* value (`resolveInvestmentAmount()` — per-stock override or the global `trading_rules.investment_amount` default), never the raw nullable `stock_mapping.investment_amount` column.

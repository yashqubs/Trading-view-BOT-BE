...
|---|---|
| Auth / 2FA / sessions | Section 5 Layer 4 (incl. "Session recovery"), Section 6 (User Management), Section 8 users table |
| Closing positions manually | Section 10 "Close all positions (manual)", Section 15 endpoint 7 |
...
| Database schema | Section 8 — every table, column, type, and all 19 trade statuses |
...
- **Short selling (added 2026-07-16): one position per ticker, at most, never hedged.** Every signal resolves the ticker's current open position first (Section 9 step 5). No position → the signal OPENS one (BUY opens a long, SELL opens a short — both are new exposure, both subject to the daily throttles). Position in the same direction already open → skipped (`ALREADY_LONG`/`ALREADY_SHORT`), so a repeated signal never doubles exposure. Position in the opposite direction → the signal **REVERSES** it: close the existing position (never throttled — blocking a close over a daily cap would leave unwanted exposure open), then, only if that close came back SUCCESS, open a fresh position in the signal's direction. That reopen is new exposure, so it runs the daily throttles like any open; a throttled reopen leaves the ticker **flat** rather than reversed, and a failed close means no reopen is attempted at all (never open on top of an uncertain state). **A reversal writes two `trade_log` rows**, so anything counting trades per signal must expect that. `TradeService.executeTrade` decides open-vs-close from whether `existingPosition` is null, NOT from `input.direction` — either direction can now open or close depending on what's already open. This reverses the pre-2026-07-16 rule that a SELL with no position always skips.
...
- **`POST /webhook/signal` overrides the global strict ValidationPipe on purpose** — it declares `ValidationPipe({ whitelist: true, forbidNonWhitelisted: false, transform: true })` on its `@Body()`, unlike every other route (main.ts sets `forbidNonWhitelisted: true` globally). The alert message is edited in TradingView, not in this repo, so a new field lands on a running server with no deploy; under the strict rule that's a `400`, and a rejected webhook is a whole trade lost with no `trade_log` row to explain it. `whitelist: true` still strips undeclared properties before the signal pipeline sees them. Don't "tidy" this back to the global pipe. Only `secret`/`ticker`/`action`/`price` drive the trade — `interval`, `time`, `indicator` (added to the template 2026-08-01) are optional, loosely validated, and unused; notably `time` (the bar's open time) is *not* `signal_received_at`, which stays the server's receipt clock. Section 14.
- **`trade_log.position_opened_at` has exactly one chance to be captured** — while the position still exists. On a close it's IG's open time for the position being closed (`IgPosition.openedAt`, set into `baseLog` in `executeTrade` and `closePositionAtMarket`); on a successful open `saveSuccess` stamps the fill's own time. It's what lets the portal say how long an exposure was held, since the matching open row may predate this bot entirely. Never back-fill it from a matching opening row — that's wrong for anything opened outside the bot and indistinguishable from a real value once written.
- **IG's `position.createdDateUTC` is UTC with no offset on the string** (`2026-08-01T02:23:00`), so passing it straight to `new Date()` parses it as local time and lands every position's open time hours out, silently. `toIsoUtc` in `ig-client.service.ts` appends `Z` first; it feeds `IgPosition.openedAt` → `OpenPosition.openedAt` → the portal's "Opened" column. IG's other field, `createdDate`, is in the account's unnamed dealing time zone and is deliberately never used as a fallback — a missing open time stays `null` rather than becoming a confident guess. Section 15 endpoint 6.
...
- **A lapsed session must never leave a user unable to log back in** (Section 5 Layer 4 "Session recovery"). No user should ever have to clear cookies by hand; if that is ever the fix again, something below has been undone. Four things guarantee it, none optional — see `auth/session/session-recovery.spec.ts`, which asserts the whole chain over real HTTP:
  1. `CsrfGuard` enforces the double-submit check only when the `access_token` cookie is a **live** session (`isLiveSessionToken` — well-formed JWT, `exp` in the future), not merely present. This is the one that kept the lockout alive after the first two attempts at fixing it: `CsrfGuard` is a global guard, so it runs *ahead* of every route's `JwtAuthGuard`, and a stale cookie made it answer **403** — which short-circuits the request, so the 401-driven cleanup in (3) never ran and the jar could never heal. Waiving is safe because every non-exempt state-changing route is JWT-guarded, so those requests still 401; they just get there honestly.
  2. `POST /auth/logout` is deliberately **not** behind `JwtAuthGuard` and is CSRF-exempt, and `clearAllSessionCookies` clears host-only *and* domain-scoped variants of all three cookies. It is the last-resort escape hatch, so it has to work on a jar too broken to authenticate with and leave it genuinely empty. The frontend calls it (`clearServerSession`) whenever it gives up on a session and again on the login page — nothing in the browser can delete an httpOnly cookie, so this call is the only thing that ends a poisoned jar.
  3. `GlobalExceptionFilter` clears a rejected `access_token` (and both `csrf_token` variants) on a 401 — but only when no `refresh_token` cookie remains, and never for `/auth/refresh` or `/auth/login`. That condition is what makes it safe: an expired access token with a live refresh token is the normal 15-minute state, and clearing on a slow 401 that landed after a successful refresh would wipe the fresh cookies.
  4. The same filter clears the access half on a **CSRF 403**, unconditionally this time. A csrf cookie/header mismatch is unrepairable from the client (it can neither read the httpOnly half nor mint a replacement), so every mutation 403s identically until the cookie expires. Dropping the access half turns the next request into a 401 the refresh cookie renews silently, reissuing both cookies in sync. No refresh race applies here — a renewed session gets a matching pair on the retry. Matched on the message so `JwtAuthGuard`'s other 403 ("finish setting up 2FA", a healthy session) is left alone.
- **Duplicate cookies are a real failure mode, not a theoretical one.** `cookie-parser` keeps the **first** of two same-named cookies, and a browser sends a host-only leftover ahead of the domain-scoped one (equal path length, older creation time). So a stale host-only `csrf_token` permanently shadows the live `.qubs.co.uk` one that the portal's JS reads and echoes back — verified against production. Anything that clears one of these cookies must clear both variants.
- The refresh-token idle window (`REFRESH_TOKEN_TTL_MS`) is **1 hour, deliberately** — decided 2026-07-27 after explicitly weighing 7- and 30-day alternatives. Being asked to sign in again after a gap is expected behaviour on a real-money portal, not a bug. Don't raise it or add "remember me" without discussing first.
- `POST /trades/close-all-positions` (`TradeService.closeAllOpenPositions`) is the portal's manual "flatten the book" button. It runs **outside** the signal pipeline on purpose: no condition checks, no daily throttles, no `bot_enabled` gate (closing reduces exposure, and the button has to work while the bot is paused). Always MARKET, never LIMIT, whatever the stock's `execution_mode` says — a LIMIT that can't fill leaves the position open, the opposite of what the button promises. It never touches the consecutive-failure counter in either direction, so closing out after hours can't silently auto-pause the bot. Unmapped epics are closed too. Returns `{ attempted, closed, failures[] }` — a partial result is a normal outcome, not an error.
- `GET /trades` (and `/trades/export`, same DTO) accepts a **comma-separated `status` list**, not just one value — `TradeLogQueryDto.status` normalizes a single value, a comma list, or repeated `status=` keys into `TradeStatus[]`, applied as one `IN (...)` clause by `applyTradeLogFilters` (Section 10 "Multi-status filtering"). This backs the portal's "Executed only" (SUCCESS + FAILED) trade-history view — the default on `/stocks/:ticker`, an opt-in on `/trades`. A bare single status still works exactly as before.
...
- `trade/` — trade execution + trade_log writing + the manual close-all-positions path
- `email/` — AWS SES sender (OTP, invite, password reset), authorized by the EC2 IAM role

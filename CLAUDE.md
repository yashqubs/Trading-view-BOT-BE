...
- **A lapsed session must never leave a user unable to log back in** (Section 5 Layer 4 "Session recovery"). Two things guarantee that, and neither is optional: (1) `POST /auth/logout` is deliberately **not** behind `JwtAuthGuard` and is CSRF-exempt — logging out destroys authority rather than exercising it, and requiring a valid session to end one is backwards, since the user who most needs their cookies cleared is the one whose session already died. (2) `GlobalExceptionFilter` clears a rejected `access_token` (and `csrf_token`) on a 401 — but only when no `refresh_token` cookie remains, and never for `/auth/refresh` or `/auth/login`. A dead-but-present `access_token` stops `CsrfGuard` waiving the double-submit check, which locked users out entirely until they cleared cookies by hand. The refresh-cookie condition is what makes this safe: an expired access token with a live refresh token is the normal 15-minute state, and clearing on a slow 401 that landed after a successful refresh would wipe the fresh cookies.

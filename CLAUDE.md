...
- `GET /trades` (and `/trades/export`, same DTO) accepts a **comma-separated `status` list**, not just one value — `TradeLogQueryDto.status` normalizes a single value, a comma list, or repeated `status=` keys into `TradeStatus[]`, applied as one `IN (...)` clause by `applyTradeLogFilters` (Section 10 "Multi-status filtering"). This backs the portal's default "Executed only" (SUCCESS + FAILED) trade-history view. A bare single status still works exactly as before.
